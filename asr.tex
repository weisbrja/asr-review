...
Since speech recognition systems typically require their audio input to have a fixed sampling frequency (e.g.\ $\SI{16}{\kilo\hertz}$ for wav2vec 2.0 \cite{baevskiWav2vec20Framework2020} and Whisper \cite{radfordRobustSpeechRecognition2023}) and a fixed bit depth, recorded audio data often needs to be resampled and requantized before it can be processed.
...
After discussing how the speech recognition model transforms speech features into output tokens resulting in a transcription of the input audio, we will talk about a post-processing step often applied to the generated transcriptions as a last part of the ASR pipeline.
...
To actually produce naturalistic transcripts, these systems therefore often implemented an inverse mapping of the standardized transcriptions back to conventional written text, possibly applying a specific locale in the process.
This step is often accordingly called inverse text normalization.

\paragraph{End-to-End ASR Models}
Newer end-to-end spech recognition systems, such as Whisper \cite{radfordRobustSpeechRecognition2023}, directly learn the mapping between utterances and their textual transcripts, eliminating the need for explicit inverse text normalization, thereby simplifying the speech recognition pipeline.

\section{Comparison of ASR models}\label{sec:comparison_of_models}

Having outlined the classic pipeline approach to ASR, in which neural networks---particularly Transformer-based architectures---act as the core component of a speech recognition pipeline, we now examine two representative models that illustrate different paradigms of contemporary Transformer-based ASR:
...
\paragraph{Self-Supervision in Low-Resource Scenarios}
...
\paragraph{In-Distribution vs.\ Out-of-Distribution Evaluation}
...
In practice, many ASR systems are evaluated on the LibriSpeech benchmark, which \citet{radfordRobustSpeechRecognition2023} therefore use as an anchor for in-distribution performance, with additional test sets evaluated relative to this baseline.

\paragraph{Zero-Shot Evaluation and Text Normalization instead of Fine-Tuning}
To solve the issue of inflated WER scores because of small stylistic mismatches in transcriptions without needing to rely on fine-tuning to the transcription style of each test set, \citet{radfordRobustSpeechRecognition2023} instead use a zero-shot approach with Whisper when evaluating and instead apply dataset-specific text normalization (Section~\ref{sec:inverse_text_normalization}) before calculating WER to produce semantically more accurate scores, that do not penalize minor differences in transcription style.
...
% TODO: continue adding paragraphs here
