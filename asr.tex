...
\paragraph{Vocabulary Size and Domain-Specific Language}
A major challenge in ASR is recognizing a large and potentially specialized vocabulary.
...
The presence of domain-specific language, such as medical, legal or technical terminology, further increases the challenge, as such specialized words are often underrepresented in training data and harder for models to recognize accurately.

\paragraph{Audio Quality and Environmental Factors}
Another very important dimension is given by the environment and setup used for recording the audio data to be transcribed.
...
\paragraph{Conversational Speech}
...
\paragraph{Variability in Language and Speaker Characteristics}
...
We then examine and compare two representative systems---wav2vec~2.0 \citep{baevskiWav2vec20Framework} and OpenAI's Whisper \citep{radfordRobustSpeechRecognition2023}---discussing architectures, training, inference and evaluation of their performance. % TODO: add further points discussed in this section here.
...
\citet{baevskiWav2vec20Framework} rescale the input of wav2vec~2.0 to have zero mean and unit variance, while \citet{radfordRobustSpeechRecognition2023} normalize the computed channels to lie within the range $[-1,1]$ and to also have approximately zero mean before processing them with Whisper.

\subsubsection{Text Normalization}\label{sec:text_normalization}

Historically, ASR systems were often trained to produce normalized transcriptions, stripped of any punctuation and entirely upper or lower case, using standardized spelling and with dates, numbers and symbols turned into a standardized form, e.g.\ \enquote{twenty-five} into \enquote{25} or \enquote{six dollars} into \enquote{\$6}.
These systems thus often implemented inverse text normalization---mapping the standardized transcripts back to naturalistic text, possibly using a different locale.

With Whisper, \citet{radfordRobustSpeechRecognition2023} demonstrated that modern ASR architectures are capable of directly learning the mapping between utterances and text transcripts, eliminating the need for such explicit text or inverse text normalization, thereby simplifying the speech recognition pipeline.
...
As in other areas of machine learning, neural networks have long become the dominant modeling approach, especially in NLP.

Over time, the state-of-the-art models for ASR have deployed many different architectures, ranging from feedforward to convolutional and recurrent models, ultimately converging on attention-based designs.
...
wav2vec~2.0 \citep{baevskiWav2vec20Framework}, a self-supervised encoder-only model fine-tuned with CTC loss, and OpenAI's Whisper \citep{radfordRobustSpeechRecognition2023}, a weakly-supervised encoder-decoder model.
...
This is done for all exptected transcript tokens.
...
To actually perform speech recognition, a linear decoder is appended to the context network of the pre-trained encoder-only model.
This decoder maps each contextualized speech representation to a logit vector, followed by a softmax to produce a probability distribution over the output vocabulary, e.g.\ characters or phonemes as well as a CTC boundary symbol.
Training this initially randomized decoder in a fine-tuning step is therefore a necessity in applying a pre-trained wav2vec~2.0 model to a task such as ASR.
...
The actual fine-tuning of wav2vec~2.0 performed by \citet{baevskiWav2vec20Framework} uses CTC loss \cite{gravesConnectionistTemporalClassification} with some (possibly very little) labeled speech data.
...
every output token in the output sequence is assumed to be conditionally independent of every other output token given the input sequence of audio speech features.
% TODO: should i use more math like in the following?
\begin{comment}
Every output token $a_i$ in the output sequence $a_1, \ldots, a_T$ is assumed to be conditionally independent of every other output token $a_j$ given the input sequence $X$ of audio speech features:
\begin{equation*}
  P(a_1, a_2, \ldots, a_T\mid X) = \prod_{t=1}^T P(a_t\mid X)
\end{equation*}
\end{comment}
As a result, the decoder added onto a pre-trained wav2vec~2.0 model cannot capture sequential dependencies between output tokens and does therefore not learn an implicit language model, unlike Whisper, which does not make this assumption of conditional independence.
...
Both Whisper and wav2vec~2.0 use a decoder network---albeit of quite different complexity---to convert the latent representations produced by their respective encoders into output tokens for inference.
...
Because \citet{baevskiWav2vec20Framework} assume output tokens to only depend on the latent representations generated by the encoder and be independent of each other, inference of an output token for each time step of the input audio can be done in parallel.
...
Since wav2vec~2.0 does not learn an implicit language model (Section~\ref{sec:fine-tuning}), integrating such an external language model (at least for transcript generation) is particularly beneficial \cite{baevskiWav2vec20Framework}.
For Whisper, the usage of an external language models can still improve transcript quality, as its implicit language model is trained on substantially less text data than modern LLMs \cite{jurafskySpeechLanguageProcessing2025}.
...
With wav2vec~2.0, \citet{baevskiWav2vec20Framework} trained two different model configurations: a \textsc{Base} model with 95 million parameters and a \textsc{Large} model with 317 million parameters.

After pre-training such models on either the $\SI{960}{\hour}$ of LibriSpeech audio or the roughly $\SI{60}{\kilo\hour}$ of audio data from LibriVox as described in Section~\ref{sec:training}, \citet{baevskiWav2vec20Framework} consider five datasets for fine-tuning each model.
...
These results represent both an impressive feat in speech processing using unsupervised learning, massively improving on previous work and further, they showed that effective speech recognition is even possible in ultra-low resource scenarios, possibly enabling speech recognition for many lesser spoken languages or dialects.

Increasing the amount of unlabeled audio data ($\SI{960}{\hour}$ vs. $\SI{60}{\kilo\hour}$) shows large improvements in performance, demonstrating that unsupervised speech recognition can be significantly improved by training on more unlabeled data.

Increasing model size from \textsc{Base} to \textsc{Large} also improves WER in all tested scenarios.

In a high-resource labeled data scenario, wav2vec~2.0 also proves effective, as the method's best WER of $1.8/3.3$ is achieved by the \textsc{Large} model pre-trained on the full LibriVox corpus ($\SI{60}{\kilo\hour}$ of unlabeled audio) and fine-tuned on the most amount of labeled data (the full $\SI{960}{\hour}$ from the LibriSpeech corpus) when paired with the Transformer-base language model.
This demonstrates that unsupervised learning for speech recognition is not only applicable in low-resource scenarios, rather it also compares well against supervised methods in high-resource scenarios.
...
When increasing model size, they also observe an increase in performance, similar to \cite{baevskiWav2vec20Framework}.
...
They conclude that unsupervised approaches also benefit from even more unlabeled data.
