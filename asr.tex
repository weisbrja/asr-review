...
In this review, we will describe the ASR task and the challenges in achieving high accuracy transcriptions.
We will then talk in detail about the different approaches taken by two modern ASR systems, comparing their design, goals and performance.
% TODO: historic development of performance metric
...
Before diving into the mentioned speech recognition models, we will first discuss some common techniques applied not only in the reference systems, but also many other ASR systems.
...
Other ASR systems, such as Whisper, operate on a different audio representation and therefore require an additional preprocessing step.
...
\citet{baevskiWav2vec20Framework} rescale the input of wav2vec~2.0 to have zero mean and unit variance.
With Whisper, \citet{radfordRobustSpeechRecognition2023} normalize the computed channels to lie within the range $[-1,1]$ and to also have approximately zero mean.

\subsubsection{Text Normalization}

Historically, ASR systems were often trained to produce normalized transcriptions, stripped of any punctuation and entirely upper or lower case, standardized spelling and with dates, numbers and symbols turned into a standardized form, e.g.\ \enquote{25} into \enquote{twenty-five} or \enquote{six dollars} into \enquote{\$6}.
...
% TODO: also do this section as a historical overview of the previously used architectures in ASR: DNN, CNN, RNN and nowadays Transformer
...
Both models also employ (a modified version of) SpecAugment \citet{parkSpecAugmentSimpleData2019} to augment labeled training data, with wav2vec~2.0 using it during fine-tuning (see section~\ref{sec:fine-tuning}).
...
This decoder maps each contextualized speech representation to a logit vector, followed by a softmax to produce a probability distribution over the output vocabulary, e.g.\ characters, punctuation marks or subword tokens.
...
In contrast, Whisper is already trained on large amounts of labeled speech samples and can perform ASR in zero-shot setting, making fine-tuning optional---one of the primary research goals of \citet{radfordRobustSpeechRecognition2023}.
...
Fine-tuning the decoder using CTC loss has the goal of maximizing the probability of the model outputting the correct transcript, when all duplicate output tokens, e.g.\ characters, between blank tokens are collapsed, as this collapsing also happens during inference (see section~\ref{sec:inference}).
...
\end{equation*}
\end{comment}
As a result, the decoder added onto a pre-trained wav2vec~2.0 model cannot capture sequential dependencies between output tokens and does therefore not learn an implicit language model, unlike Whisper, which does not make this assumption.

Not learning an implicit language model can partially be overcome by including an external language model when decoding (see section~\ref{sec:inference}).
This approach might even be beneficial when transferring between different ASR domains:
even though the language used in the domains might differ, the acoustics of speech often still remain similar.
With a decoder trained only using CTC, a new language model can then simply be swapped in to adapt to the new domain.

\subsubsection{Inference}\label{sec:inference}

Both Whisper and wav2vec~2.0 use a decoder network, but inference is still very different for both systems.

% TODO:
\begin{comment}
wav2vec 2.0:
  CTC inference using beam search
  language model inclusion
whisper:
  output tokens passed back in
  autoregessive: what does this mean?
  different tasks: how are they specified?
  bpe output tokens
\end{comment}
...
whisper:
  ...
  optimized for robustness and accuracy
  multi-lingual
  multi-task
\end{comment}
