...
Beyond vocabulary size, ASR must handle substantial acoustic variability:
identical sentences can be uttered in many different ways due to accent, dialect, speaking rate or prosody.

Furthermore, spoken language often contains disfluencies---fragmented, repeated or truncated words (e.g., \enquote{main- mainly}) and filled pauses (e.g., \enquote{uh}, \enquote{uhm})---which deviate from canonical word forms and increase transcription uncertainty.

\paragraph{Low-Resource Scenarios}
Accents and dialects of lesser-spoken languages pose particular challenges for ASR systems due to limited labeled speech data and underrepresentation in speech corpora.
...
\paragraph{Audio Quality and Recording Conditions}

A further dimension of complexity arises from the recording conditions.
Speech captured in a quiet studio with a high-quality microphone placed close to the speaker is far easier to transcribe than audio recorded in noisy, everyday environments such as busy streets, restaurants or echoing rooms.

Background noise and competing speakers can mask and distort the speech signal, while reverberation blurs sounds over time, reducing phonetic distinctiveness.
Audio compression or transmission artifacts can further degrade the audio signal.

Together, these factors substantially increase ASR difficulty in real-world settings.

\section{Approaches to ASR}

Having detailled the core challenges, we now turn to methodological approaches to ASR.
Over time, a broad spectrum of paradigms has been developed to address the ASR task.

\paragraph{ASR Pipeline}
Traditional ASR systems are organized as pipelines consisting of multiple components, centered around a core speech recognition model.
Typical components include audio format conversion, feature extraction, subsampling, voice activity detection and inverse text normalization \cite{radfordRobustSpeechRecognition2023}.
...
However, many of the ideas described below are still used within these systems.
